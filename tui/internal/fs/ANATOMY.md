...
| **direct_mail.go** | | |
| `DirectTarget` / `AddressFingerprint` | `tui/internal/fs/direct_mail.go:9-22` | target identity is the caller's canonical working directory plus network address; the trimmed address has a stable nickname-independent fingerprint |
| `NormalizeMailEndpoints` / `IsDirectMail` | `tui/internal/fs/direct_mail.go:24-87` | normalizes string-or-list `To` values, and recognizes strict human↔target threads from `From` + `To` only |
| **rail_unread.go / rail_unread_test.go** | | |
| `RailUnreadStatePath` / `OpenRailUnreadStore` | `tui/internal/fs/rail_unread.go:37-70` | opens `.tui-asset/rail-last-seen.json`; missing, malformed, or unsupported state is baselined from caller-supplied accepted snapshots before return |
| `SyncTargets` | `tui/internal/fs/rail_unread.go:75-111` | canonical-directory/address-fingerprint reconciliation drops disappeared targets and rebaselines new or changed identities via candidate write, then install |
| `UnreadCount` / `MarkSeen` | `tui/internal/fs/rail_unread.go:113-170` | counts incoming snapshot mail past a timestamp-plus-ID boundary; marking requires a synchronized identity and writes the candidate before replacing live state |
| `cloneRailUnreadState` / boundary helpers | `tui/internal/fs/rail_unread.go:181-282` | deep-copies every boundary ID slice; builds sorted maximum-timestamp ID sets from target→human direct mail, with mailbox-ID fallback and canonical directory keys |
| transactional regression coverage | `tui/internal/fs/rail_unread_test.go:9-116` | real `.tmp` directory blockers prove failed `MarkSeen` and `SyncTargets` writes leave both live and reopened state unchanged |
| **ledger.go** | | |
...
- **Writes human-owned/derived state** — `WriteMail` writes `human/mailbox/outbox/<mailbox-id>/`; only complete accepted `SessionCache` persistence/appends write `human/logs/session.jsonl` (`tui/internal/fs/session.go:300-361`); `RailUnreadStore` writes only its TUI-owned `.tui-asset/rail-last-seen.json` (`tui/internal/fs/rail_unread.go:37-70`).
...
- **Writes**: signal files (`.sleep`, `.suspend`, `.interrupt`, `.prompt`, `.inquiry`), human `mailbox/outbox/*`, human `.agent.json` location field, the TUI-derived human `logs/session.jsonl` replay cache only on accepted persist/append paths, and TUI-owned `.tui-asset/rail-last-seen.json` unread state (`tui/internal/fs/rail_unread.go:37-70`).

## Notes

- **Read-only for agent state.** This package is the TUI's window — it never writes agent-owned files except signal files. The kernel owns `.agent.json`, heartbeats, mailboxes, ledgers, logs. Do not add write paths for kernel-owned state. The exception is explicitly TUI-owned derived state, including `.tui-asset/rail-last-seen.json`, never an agent mailbox or kernel file (`tui/internal/fs/rail_unread.go:37-70`).
...
- **Direct mail identity boundary.** `DirectTarget` carries the inventory's canonical working directory and network address without unread-store state; `AddressFingerprint` hashes the trimmed address. `NormalizeMailEndpoints` is the shared list-first `To` parser for direct-thread membership and topology mail edges: it trims and order-preserving deduplicates recipients, while `IsDirectMail` uses only `From` + `To`, never CC.
- **Durable direct unread.** `RailUnreadStore` is a caller-snapshot boundary: it does no scanning or wall-clock advancement, accepts only incoming target→human direct mail, and persists a canonical-directory + address-fingerprint identity with a maximum timestamp plus mailbox-ID/ID set (`tui/internal/fs/rail_unread.go:113-282`). Nickname-only changes retain the boundary; address changes, disappearance, and directory reuse rebaseline during `SyncTargets` (`tui/internal/fs/rail_unread.go:75-111`). `SyncTargets` and `MarkSeen` deep-clone, write a candidate, then install it, so failed writes change neither live nor durable state (`tui/internal/fs/rail_unread.go:75-111`, `tui/internal/fs/rail_unread.go:161-218`).
