...
| **location.go** | | |
| `humanLocationManifestMutex` / `storeResolvedHumanLocationLocked` | `tui/internal/fs/location.go:24-31`, `tui/internal/fs/location.go:112-127` | serializes one lexical-canonical manifest path across freshness check, optional lookup, latest-manifest reread, merge-only `location` commit, and shared unique-temp atomic replacement |
| `ResolveLocation()` | `tui/internal/fs/location.go:34` | queries `ipinfo.io/json` → `Location` |
| `LocationStale(loc, maxAge)` | `tui/internal/fs/location.go:63` | true if `ResolvedAt` exceeds `maxAge` |
| `UpdateHumanLocation` / `StoreResolvedHumanLocation` | `tui/internal/fs/location.go:79-110` | coalesces stale same-manifest lookups; the resolved-value entry point synchronously reuses an existing lookup through the identical transaction |
| **network.go** | | |
