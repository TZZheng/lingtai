...
- **`mail.go:112-1219`** — `MailModel`. The network home screen — async message thread, compose input, slash-command palette, heartbeat pulse, and agent status bar. Constructor: `NewMailModel` (`mail.go:154`). Refresh/poll ticks, message history pagination, and the `/human` compose path that writes to `human/mailbox/outbox/`.
