...
- **`tui/internal/config/tui_updater.go:1-159`** — `TUIUpdater` interface plus `RunTUIUpdate` backend routing. The Homebrew backend runs the existing `brew upgrade lingtai-ai/lingtai/lingtai-tui` flow, with doctor's optional preceding `brew update` (`IncludeHomebrewUpdate`) and full-path command reporting (`ResolveHomebrewPath`); source/user-local and unknown installs emit guidance only in this stage. Callers keep version checks/prompting; the backend owns only the mutation-or-guidance for one install method.
