...
- **`lingtai-tui doctor` subcommand** (`tui/main.go:978-1018`): runs `config.RunDoctorUpdate` (`tui/main.go:990`) with both `ForceTUI=true` and `ForcePython=true`, then refreshes presets, utility skills, and `commands.json`. The report includes native file-search sidecar / Rust toolchain diagnostics (`config.checkFileSearchNative`). Designed to be usable when the TUI cannot start (broken venv, missing migrations) — it never touches `.lingtai/`. Exit nonzero only on unrecoverable failures.
- **`lingtai-tui self-update` subcommand** (`tui/main.go:1020-1041`): runs `config.RunManualTUIUpdate` (`tui/main.go:1027`) through the detected install-method backend. Homebrew installs run the brew updater; source/user-local installs run the source updater backend (`install.sh --update`); unknown installs produce unsupported guidance without trying brew.
...
- **TUI binary upgrade:** `tui/main.go:105-116` detects the current install method (`config.SelectTUIUpdater(install)`) and delegates newer-release handling to `upgrade.go` (`handleTUIUpgrade`) only for Homebrew installs; with confirmation, it puts agents in their projects to sleep, stops those TUI processes, runs the Homebrew backend, then asks the user to relaunch. Non-Homebrew installs are reported and left for manual update.
